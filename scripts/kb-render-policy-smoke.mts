...
ok("agent(owner) CANNOT publish", authorize(agentOwner, "kb.publish", { treeGranted: true }).reason === "human_only");
ok("agent(owner) CANNOT share", authorize(agentOwner, "kb.share", { treeGranted: true }).reason === "human_only");
...
ok("agent(staff) has no publish cap at all", !capabilitiesFor(agentStaff).has("kb.publish"));
